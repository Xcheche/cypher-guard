...
from accounts.forms import DashboardUpdateForm, LoginForm, UserUpdateForm
from .models import *
...
def user_login(request: HttpRequest):
    ...
    reCAPTCHA is validated inside LoginForm before authenticate() runs.
    Google OAuth uses social:begin and never hits this POST handler.
    """
    if request.method == "POST":
        ...
        if not form.is_valid():
            messages.error(
                request,
                "Invalid login details or reCAPTCHA verification failed.",
            )
    ...
    if request.method == "POST":
        ...
        if user:
            ...
            reset_url = request.build_absolute_uri(
                reverse("verify_password_reset_link")
                + f"?email={email.lower()}&token={token.token}"
            )
